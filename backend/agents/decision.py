...
def _build_regime_protocol_block(asset_class: str = "equity") -> str:
    ...
    lines = [
        ...
        "  - [MACRO]:        win-rate 5% su 19 run — la 'bestia nera'. Causa",
        "                    diagnosticata: il bot tratta i dati macro come",
        "                    news geopolitiche, cercando un nesso causale",
        "                    diretto ('inflazione scende -> mercati salgono').",
        "                    Ma il macro e' fatto di ASPETTATIVE e CORRELAZIONI",
        "                    INVERSE: spesso 'good news is bad news' (il mercato",
        "                    anticipa le banche centrali). Fix: leggere i FLUSSI",
        "                    inter-market (dollaro/yields/rotazione settoriale)",
        "                    invece dei titoli dei giornali. Vedi Macro-",
        "                    Correlation Protocol in STEP 2/2.5.",
        "",
        ...
        "",
        "  [MACRO]       finestra di evento macro SCHEDULATO (FOMC week,",
        "                CPI/PPI nelle 24h, GDP, NFP). NON 'aspetti l'esito'",
        "                per indovinarlo: leggi le CORRELAZIONI INTER-MARKET",
        "                (dollaro, yields, rotazione settoriale) e la REAZIONE",
        "                del mercato dopo il dato. Il macro NON e' un nesso",
        "                causale lineare, e' un sistema di aspettative.",
        "",
        ...
        "",
        "  [TREND-DOWN]  Lo SHORT e' il trade PRIMARIO, non solo 'difesa'.",
        "                Conviction = floor profilo, SL stretto. Apri SHORT sui",
        "                RIMBALZI verso resistenza o sui BREAKDOWN di supporto con",
        "                volume; NON shortare DENTRO un supporto gia' testato piu'",
        "                volte / molto ipervenduto (rischio rimbalzo) — li' aspetta",
        "                il rimbalzo e shorta QUELLO. 'Chiusure/hedge' vale per i",
        "                LONG residui; con esposizione long ~zero, sfruttare il",
        "                trend = SHORT, non cash. Niente long contro-trend.",
        "",
        ...
        "",
        "  [MACRO]       MACRO-CORRELATION PROTOCOL (revisione post 19 run, 5%).",
        ...
        "                  • UUP (proxy dollaro DXY): se UUP ha RS5d/20d > 0",
        "                    (dollaro forte/in rafforzamento) → VIETATO aprire",
        "                    long su equity rischioso o crypto. Preferisci",
        "                    difensivi/value o NO_TRADE.",
        "                  • TLT/IEF (proxy INVERSO dei rendimenti): se TLT ha",
        "                    RS5d/20d < 0 (bond scendono = rendimenti SALGONO)",
        "                    → VIETATO growth/tech ad alta duration (XLK,",
        "                    NVDA-like). Favorisci value/energy/financials.",
        "                  Regola: NON e' un taglio percentuale, e' un GATE",
        "                  direzionale. Se dollaro forte E yields in salita,",
        "                  il long-growth e' semplicemente fuori tavolo.",
        "",
        "                PILASTRO 2 — REACTION OVER PREDICTION (obbligatorio).",
        "                VIETATO pre-positioning sull'anticipazione del dato",
        "                ('compro prima del CPI sperando sia basso'). Non",
        "                prevedere cosa fara' la Fed: guarda come il mercato",
        "                ha DIGERITO la notizia. Opera solo DOPO che il dato",
        "                e' uscito E il mercato ha reagito (prima candela",
        ...
        "                economico DEDOTTA dal rotation scan, non con un'idea",
        "                a priori. Se i 3 pilastri non convergono → NO_TRADE.",
        "",
    ...
    if is_crypto:
        strategy_lines = [
            ...
            "",
            "  [TREND-DOWN] Lo SHORT e' il trade PRIMARIO: un trend-down strutturale",
            "                (post-ATH, funding negativi, lower-low MTF) si SFRUTTA",
            "                shortando, non solo difendendosi/stando in cash. Apri",
            "                SHORT sui RIMBALZI verso resistenza o sui BREAKDOWN con",
            "                volume (gli alts soffrono 2-3x in bear = short piu'",
            "                redditizi ma meno liquidi: occhio allo slippage). NON",
            "                shortare DENTRO un supporto gia' testato piu' volte / RSI",
            "                molto ipervenduto / Fear estrema (rischio squeeze): li'",
            "                aspetta il rimbalzo verso resistenza e shorta QUELLO,",
            "                oppure shorta il breakdown del supporto con volume. NON",
            "                aggiungere long sperando nel rimbalzo; bottom-fishing",
            "                long solo dopo capitulation (= CRASH-RALLY). Stare in",
            "                cash in un down pulito = trade MANCATO, non disciplina.",
            "",
        ...
        strategy_lines = [
            ...
            "",
            "  [MACRO]      Focus: MACRO-CORRELATION PROTOCOL — leggi i FLUSSI,",
            "                non i titoli. Il fallimento storico (5% win-rate)",
            "                veniva dal cercare nessi causali lineari. Ora il",
            "                metodo e' inter-market: il rotation scan che hai",
            "                in contesto CONTIENE GIA' tutti i segnali macro.",
            "",
            "                STEP A — DEDUCI LA FASE DEL CICLO dal rotation scan.",
            "                Non serve un calendario macro: la rotazione",
            "                relativa dei settori TI DICE in che fase sei.",
            "                Guarda quali categorie hanno rotation_score in",
            "                cima e quali in coda:",
            "                  • Energy/Commodities/Gold guidano (XLE, DBC, GLD",
            "                    in top, RS positivi) → INFLATION / LATE-CYCLE.",
            "                    Priorita': XLE, XOM, CVX, GLD, DBC. Evita",
            "                    growth-tech a lunga duration.",
            "                  • Tech/Discretionary guidano (XLK, XLY in top)",
            "                    → EXPANSION / EARLY-CYCLE. Priorita': XLK,",
            "                    XLY, semis — MA solo se yields NON in salita",
            "                    (vedi Pilastro 1: se TLT scende, declassa).",
            "                  • Healthcare/Utilities/Staples guidano (XLV,",
            "                    XLU, XLP in top) → RECESSION / RISK-OFF.",
            "                    Priorita': XLV, XLU, XLP, difensivi puri.",
            "                  • Bond lunghi (TLT) in top + UUP forte →",
            "                    FLIGHT TO SAFETY. Priorita': riduci rischio,",
            "                    cash/hedge, eventualmente TLT stesso. NO",
            "                    nuovi long equity rischiosi.",
            "",
            "                STEP B — DOLLAR & YIELDS ANCHOR (gate, da STEP 2).",
            "                  • UUP RS positivo (dollaro forte) → gate sui",
            "                    long equity/crypto rischiosi.",
            "                  • TLT RS negativo (yields salgono) → gate sui",
            "                    growth/tech ad alta duration.",
            "                Il trade DEVE sopravvivere a entrambi i gate.",
            "",
            "                STEP C — REACTION OVER PREDICTION.",
            "                NON anticipare il dato. Entra solo dopo che il",
            "                mercato ha reagito, nella DIREZIONE confermata",
            "                dalla prima candela completa + volume. 'Good news",
            "                is bad news' e' normale nel macro: fidati del",
            "                prezzo post-evento, non della tua tesi pre-evento.",
            "",
            "                REGOLA FINALE: i tre step devono CONVERGERE. Se la",
            "                fase del ciclo dice 'Energy' ma il dollaro forte",
            "                gate-a l'energy USD-denominated, oppure la reazione",
            "                post-dato e' ambigua → NO_TRADE. Nel macro la",
            "                coerenza inter-market vale piu' di ogni singola tesi.",
            "",
            "  [CRASH-RALLY] Focus: CONVEXITY & SPEED — emozione domina.",
            "                In questo regime domina paura/FOMO. La razionalita'",
            "                svanisce. NON cercare di comprare il minimo o vendere",
    ...
    lines.extend([
        ...
        "",
        "  A2) Se hai classificato [MACRO] e decidi di entrare, DEVI",
        "      scrivere ESPLICITAMENTE prima del tool execute_trade la",
        "      verifica dei 3 pilastri del Macro-Correlation Protocol:",
        "        \"Deroga dal NO_TRADE default — Macro-Correlation check:",
        "           (1) DOLLAR&YIELDS ANCHOR: UUP RS5d = [val] (dollaro",
        "               [forte/debole]), TLT RS5d = [val] (yields [su/giu']).",
        "               Il trade [ticker] sopravvive a entrambi i gate",
        "               perche' [...],",
        "           (2) FASE CICLO da rotation scan = [INFLATION/EXPANSION/",
        "               RECESSION/FLIGHT-TO-SAFETY], dedotta da [categorie",
        "               leader]. Il ticker e' coerente con la fase perche' [...],",
        "           (3) REACTION OVER PREDICTION: il dato [CPI/Fed/NFP] e'",
        "               [gia' uscito da Xh / non imminente], la reazione del",
        "               mercato e' [direzione confermata da candela+volume].",
        "               NON sto anticipando, sto seguendo la reazione.",
        "           (4) conviction = X.XX > " + boosted_conf_str + ", SL = Y%",
        "               su livello tecnico.\"",
        "      Se anche UNO dei 3 pilastri non e' soddisfatto/allineato,",
        "      il trade NON e' ammesso → NO_TRADE.",
        "",
...
async def _handle_decision_tool(tool_name: str, tool_input: dict, run_id: str,
    ...
    try:
        ...
        if tool_name == "execute_trade":
            ...
            # 13 inversioni di direzione sullo stesso titolo entro 72h su 51
            # operazioni. Blocca solo il rientro dopo uno stop e il
            # ribaltamento rapido; le chiusure passano sempre (vedi
            # churn_guard.py). Fail-open per costruzione.
            if action in ("BUY", "SHORT"):
                ...
                if not _churn_ok:
                    logger.warning("[%s][DECISION] CHURN_GUARD blocked: %s",
                                   run_id, _churn_why)
                    database.insert_agent_log(run_id, "DECISION_CHURN_BLOCKED",
