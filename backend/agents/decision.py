...
def _build_regime_protocol_block(asset_class: str = "equity") -> str:
    ...
    if is_crypto:
        strategy_lines = [
            ...
            "",
            "  [CRASH-RALLY] Focus: posizioni anti-fragili.",
            "                In shock di volatilita' (VIX > 30 + crypto down >10%):",
            "                attendi il primo bar di stabilizzazione, NON prendere",
            "                il primo coltello. In capitulation rebound, BTC guida e",
            "                gli alts seguono con beta 1.5-3x. Strategia tipica:",
            "                long BTC al primo segnale di reversal su volumi spike,",
            "                size piccola, SL standard del profilo (non stretti: la",
            "                volatilita' di noise stoppa SL stretti). Mai over-",
            "                leverage in questi regimi: liquidations chain trigger.",
        ]
    else:
        strategy_lines = [
            ...
            "                Dato critico dell'analisi Simulator: i breakout in regime",
            "                laterale sono FALSI nel ~70% dei casi → il bot ha perso",
            "                sistematicamente comprando rotture di range che rientrano.",
            "                Strategia corretta: compra SOLO vicino ai supporti storici",
            "                del range, vendi SOLO vicino alle resistenze. Logica",
            "                contrarian, non momentum.",
            "                  • Size del trade RIDOTTA del 50% vs normale.",
            "                  • Take-profit AGGRESSIVI: chiudi all'altra estremita'",
            "                    del range, non aspettare il 'breakout vero'.",
            "                  • Stop-loss STRETTI: se il supporto cede, esci subito",
            "                    (non e' piu' laterale → ri-classifica regime).",
            "                Se il prezzo e' a META' del range, UNICA azione ammessa",
            "                e' HOLD o NO_TRADE. Niente trade a metà range.",
            "",
            ...
            "                In shock di volatilita' (VIX > 30): quality + cash,",
            "                evita ciclici e leverage. In capitulation rebound,",
            "                cerca i settori 'battuti peggio' che mostrano i primi",
            "                segnali di reversal (high-beta in relief rally: Tech",
            "                growth, Small caps IWM). Size piccola, SL standard del",
            "                profilo (no stop stretti: noise alto). NO over-leverage:",
            "                il rebound puo' essere fake-out su corto orizzonte.",
        ]
