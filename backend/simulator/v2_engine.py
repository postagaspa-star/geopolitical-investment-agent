...
async def start_run(
    ...
    # Pre-fetch dei prezzi di T0 per validare lo scenario
    t0 = step_dates[0]
    universe = scenario.get("asset_universe", [])
    t0_prices = await fetch_prices_at_date(universe, t0)
    if not t0_prices:
        # Polygon vuoto per la data → fallback ai market_data hardcoded
        # (es. scenari pre-2020 o ticker non Polygon)
        logger.warning("[SIM-V2] Nessun prezzo Polygon per %s, uso market_data hardcoded",
                       scenario["id"])
        t0_prices = {m["ticker"]: float(m["price_t0"])
    ...
    return {
        "scenario": {
            ...
            # Headlines: divise tra gli step (round-robin se non ci sono enough)
            "headlines_master": scenario.get("headlines", []),
            # description_reveal NON inviato al client — solo nel debrief finale
        },
        "portfolio": portfolio,
        "t0_prices": t0_prices,
...
async def execute_step(
    ...
    """
    Esegue lo step N:
      1. Fetcha prezzi reali Polygon a step_dates[step_index]
      2. Costruisce contesto + chiama AI
      3. Applica trade al portfolio
      4. Ritorna step_data + nuovo portfolio
    """
    ...
    # 1. Prezzi reali al target_date
    prices = await fetch_prices_at_date(universe, target_date)
    if not prices:
        # Fallback: usa prezzi precedenti o hardcoded
        if history and history[-1].get("prices"):
            prices = history[-1]["prices"]
        else:
            prices = {}
    ...
    user_msg = _build_step_message(
        scenario, portfolio, valuation_before, prices, headlines,
        history, step_index, num_steps, target_date
    )

    # 5. Call AI
    raw = await _call_r1(SIM_V2_SYSTEM_PROMPT, user_msg)
...
def _build_step_message(
    ...
    prices: dict, headlines: list[str], history: list[dict],
    step_index: int, num_steps: int, target_date: str
) -> str:
    """Costruisce il messaggio user per il modello al turno step_index."""
    parts = []
    ...
    if step_index == 0:
        ...
        parts.append(f"Data simulata: {target_date}")
        parts.append(f"È passata 1 settimana dal turno precedente.")
    parts.append("═" * 60)
    parts.append("")
    ...
    if valuation.get("positions"):
        ...
        for p in valuation["positions"]:
            ...
            parts.append(
                ...
                f"@ ${p['avg_entry_price']:.2f} (ora ${p['current_price']:.2f}) "
                f"P&L: {sign}{p['unrealized_pnl_pct']:.2f}%"
            )
    else:
        parts.append("  Posizioni aperte: nessuna")
    parts.append("")

    # Asset universe + prezzi correnti
    parts.append(f"🎯 ASSET DISPONIBILI (prezzi reali al {target_date}):")
    for ticker in scenario.get("asset_universe", []):
        if ticker in prices:
            parts.append(f"  {ticker:8s}  ${prices[ticker]:>9.2f}")
        else:
            parts.append(f"  {ticker:8s}  (prezzo non disponibile)")
    parts.append("")

    # Headlines
    if headlines:
        parts.append("📰 HEADLINE (questa settimana):")
        for h in headlines:
            parts.append(f"  • {h}")
        parts.append("")
    ...
    if step_index > 0 and history:
        ...
        for h in history[-3:]:   # ultimi 3 turni
            ...
            if trades:
                ...
                parts.append(f"  Turno {t_idx + 1} ({t_date}): nessuna azione")
        parts.append("")
...
async def finalize_run(
    ...
    Chiude la simulazione:
      1. Fetcha prezzi finali (data dell'ultimo step + 1 settimana)
      2. Calcola P&L finale
      3. Genera debrief AI
      4. Salva nel DB se persist=True
    """
    step_dates = scenario.get("step_dates", [])
    if not step_dates:
        raise ValueError("scenario senza step_dates")
    final_date = step_dates[-1]   # T+Nw (ultimo elemento di step_dates)
    universe = scenario.get("asset_universe", [])

    final_prices = await fetch_prices_at_date(universe, final_date)
    ...
    # Benchmark SPY: long SPY all'inizio della partita
    t0_date = step_dates[0]
    spy_prices = await fetch_prices_at_date(["SPY"], t0_date)
    spy_t0 = spy_prices.get("SPY")
    spy_final = final_prices.get("SPY") or (await fetch_prices_at_date(["SPY"], final_date)).get("SPY")
    benchmark_pnl_pct = None
    if spy_t0 and spy_final:
        benchmark_pnl_pct = round((spy_final - spy_t0) / spy_t0 * 100, 2)

    # Description reveal (cosa è successo davvero)
    from simulator import scenarios as _scen
    full_scenario = _scen.get_scenario_by_id(scenario.get("id", ""))
    reveal = full_scenario.get("description_reveal", "") if full_scenario else ""

    # Debrief AI: una sola chiamata leggera
